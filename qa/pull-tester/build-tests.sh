...
if [ -d "$OUT_DIR" -a -w "$OUT_DIR" ]; then
  set +e
  # Windows:
  cp /D/FTC/dev/ftc_0.9.3/win32-build/src/bitcoind.exe $OUT_DIR/bitcoind.exe
  cp /D/FTC/dev/ftc_0.9.3/win32-build/src/test/test_bitcoin.exe $OUT_DIR/test_bitcoin.exe
  cp /D/FTC/dev/ftc_0.9.3/win32-build/src/qt/bitcoind-qt.exe $OUT_DIR/bitcoin-qt.exe
  # Linux:
  cp /D/FTC/dev/ftc_0.9.3/linux-build/src/bitcoind $OUT_DIR/bitcoind
  cp /D/FTC/dev/ftc_0.9.3/linux-build/src/test/test_bitcoin $OUT_DIR/test_bitcoin
  cp /D/FTC/dev/ftc_0.9.3/linux-build/src/qt/bitcoind-qt $OUT_DIR/bitcoin-qt
  set -e
fi

# Run unit tests and blockchain-tester on Linux:
cd /D/FTC/dev/ftc_0.9.3/linux-build
make check

# Run RPC integration test on Linux:
/D/FTC/dev/ftc_0.9.3/qa/rpc-tests/wallet.sh /D/FTC/dev/ftc_0.9.3/linux-build/src
/D/FTC/dev/ftc_0.9.3/qa/rpc-tests/listtransactions.py --srcdir /D/FTC/dev/ftc_0.9.3/linux-build/src
# Clean up cache/ directory that the python regression tests create
rm -rf cache

if [ $RUN_EXPENSIVE_TESTS = 1 ]; then
  # Run unit tests and blockchain-tester on Windows:
  cd /D/FTC/dev/ftc_0.9.3/win32-build
  make check
fi
